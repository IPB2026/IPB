...
export async function recordFacturePayment(formData: FormData) {
  ...
  if (soldee) {
    ...
    if (!dejaEnvoye && !dejaRelance) {
      // Le paiement DÉCLENCHE la rédaction : la tâche doit être actionnable TOUT
      // DE SUITE (dueAt = maintenant) → elle apparaît immédiatement dans « Relances
      // dues » comme à-faire, et plus elle y reste, plus le retard sur la promesse
      // « 3 à 5 jours ouvrés » est visible.
      await prisma.activity.create({
        data: {
          type: 'RELANCE',
          contactId: f.contactId,
