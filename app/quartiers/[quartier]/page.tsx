...
export default function QuartierPage({ params }: PageProps) {
  const { quartier } = params;
  const quartierInfo = quartiersData[quartier];

  if (!quartierInfo) {
    return <div>Quartier non trouvé</div>;
  }
      ...
      <div className="bg-white border-b border-slate-200">
        <div className="max-w-7xl mx-auto px-4 py-3">
          <nav className="flex items-center gap-2 text-sm text-slate-600">
            ...
            <span>/</span>
            <span className="text-slate-900 font-medium">{quartierInfo.nom}</span>
          </nav>
        </div>
      </div>
